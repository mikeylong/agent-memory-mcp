...
print_help() {
  cat <<'EOF'
Usage: import-codex-session.sh [options]

Import a Codex session into agent-memory-mcp.
If --session-file is omitted, the latest Codex session file is selected automatically.

Options:
  --session-file <path>   Codex session .jsonl file (optional)
...
Examples:
  import-codex-session.sh
  import-codex-session.sh --project-path "$HOME/projects/agent-memory"
  import-codex-session.sh --scope session --session-id codex-replay-01
  import-codex-session.sh --session-file "$HOME/.codex/sessions/YYYY/MM/DD/rollout-<id>.jsonl"
EOF
}
...
if [[ -z "$SESSION_FILE" ]]; then
  if ! SESSION_FILE="$(find_latest_session_file "$SOURCE_ROOT")"; then
    echo "Error: No Codex session .jsonl files found under $SOURCE_ROOT." >&2
...
echo "Using Codex session file: $SESSION_FILE"

cmd=(
  npm run -s import:codex-session --
